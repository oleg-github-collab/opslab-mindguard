...
Система одноразового зв'язування Telegram ID через **email + 4-значний код доступу**.

> ⚠️ Оновлення: генерація PIN на dashboard більше не використовується. Привʼязка: `/start email@opslab.uk 1234`.

---

## 🆕 Оновлений user flow

1. Адміністратор видає користувачу email + 4-значний код доступу.
2. Користувач пише боту: `/start email@opslab.uk 1234`.
3. Бот перевіряє код (argon2) і привʼязує Telegram ID.
4. Готово: доступні чекіни, /weblogin та налаштування.
...
### 1. Database
**Файл:** `migrations/01_init_schema.sql`
- Поле `users.telegram_id` (UNIQUE) для одноразової привʼязки
- Таблиця `telegram_pins` залишена як legacy (не використовується)

### 2. Database Functions
**Файл:** `src/db/mod.rs`
- `link_telegram_by_email_code()` - перевіряє код доступу та звʼязує Telegram ID
- `verify_and_link_telegram()` - legacy PIN flow (не використовується)

### 3. Bot Handler
**Файл:** `src/bot/enhanced_handlers.rs`
- `/start email code` - команда для зв'язування
- `handle_link_by_code()` - обробка email + коду
- Персоналізоване привітання після успішного зв'язування

### 4. Web API Endpoints
**Файл:** `src/web/telegram.rs`
- `GET /telegram/status` - перевіряє статус підключення
- `POST /telegram/preferences` - налаштування часу/поясу
