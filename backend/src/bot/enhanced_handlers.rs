...
fn parse_link_command(text: &str) -> Option<(String, String)> {
    ...
    if !(cmd_base == "/start" || cmd_base == "/link") {
        return None;
    }
    ...
    let Some(user) = user else {
            ...
            if let Some(cmd) = command.as_ref() {
                if cmd.name == "/start" || cmd.name == "/link" {
                    ...
                    bot.send_message(
                        msg.chat.id,
                        mdv2(format!(
                            "👋 Вітаю! Я допоможу швидко увімкнути Mindguard.\n\n\
                            Для привʼязки потрібні email та 4-значний код доступу.\n\
                            Це не пароль від пошти — лише короткий код, який видає адміністратор.\n\n\
                            Можна одразу надіслати так:\n\
                            /start email@opslab.uk 1234\n\n\
                            Або:\n\
                            /link email@opslab.uk 1234\n\n\
                            Якщо зручно, просто напишіть: email@opslab.uk 1234\n\n\
                            Веб-платформа: {base_url}",
                        )),
                    )
                    .parse_mode(teloxide::types::ParseMode::MarkdownV2)
                    .await?;
        ...
        bot.send_message(
            ...
            mdv2(format!(
                ...
                Я мʼяко допоможу налаштувати щоденні чекіни та персональні інсайти.\n\n\
                🧠 Що це за платформа?\n\
                Mindguard — система для моніторингу та підтримки ментального здоров'я команди.\n\n\
                🔐 Як почати (2 хв):\n\
                1. Відкрийте web: {0}\n\
                2. Увійдіть (email + 4-значний код доступу)\n\
                3. Пройдіть онбординг і встановіть час нагадувань\n\
                4. Поверніться в бот і привʼяжіть Telegram:\n\
                   /link email@opslab.uk 1234\n\n\
                💡 Хочете швидше? Просто надішліть сюди:\n\
                email@opslab.uk 1234\n\n\
                Код доступу видає адміністратор. Це не пароль від пошти.\n\
...
async fn send_start_message(bot: &teloxide::Bot, chat_id: ChatId) -> Result<()> {
    ...
    bot.send_message(
        ...
        mdv2(format!(
            ...
            Допомагаю тримати баланс між роботою та життям через:\n\n\
            ✅ Швидкі щоденні чекіни (2-3 хв)\n\
            📊 Аналітику вашого ментального здоров'я\n\
            💬 Командний feedback та підтримку\n\
            🎯 Персональні інсайти та рекомендації\n\n\
            🔗 Як почати?\n\n\
            Якщо ви ще не привʼязали Telegram:\n\
            Надішліть email та 4-значний код (не пароль!):\n\n\
            ...
            /pulse — Анонімні кімнати для обговорень\n\
            /settings — Налаштувати час нагадувань\n\
            /help — Повна допомога\n\n\
            🚀 Швидкий старт:\n\n\
            1️⃣ Привʼяжіть Telegram через /link\n\
            2️⃣ Відкрийте dashboard: {base_url}\n\
            3️⃣ Налаштуйте зручний час для чекінів\n\
            4️⃣ Отримуйте нагадування та відстежуйте прогрес\n\n\
            💡 Все конфіденційно, безпечно та анонімізовано для команди.",
        )),
    )
    .parse_mode(teloxide::types::ParseMode::MarkdownV2)
    .await?;
